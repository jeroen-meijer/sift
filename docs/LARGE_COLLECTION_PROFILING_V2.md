...
| Item | Value |
| --- | --- |
| Session | `bun run tauri:profile` (~18:47–19:00 CET) |
| Log | `logs/sift-profile.log` (76 678 lines, ~3.9 MB) |
| Ended | User quit app ~19:00 |
| Earlier dogfood | ~18:08–18:25; see [Archive](#archive-first-dogfood-1813) |
...
- **Hydrate → analyze:** watch / availability IPC flip to `local` → `enqueue_ids`.
...
## Session outcomes (~18:47–19:00)

| Metric | Value |
| --- | --- |
| Availability backfill | **155 ms**, 1731 became local |
| Main analyze wave | start n=12609 → done **n=13770** |
...
| Scroll `q_wait` | often **150–500+** (`MAX_INFLIGHT=2`) |
| RSS | **~2.8 GB → ~12 GB** during analyze; stayed multi‑GB |

Scroll (waves ON, folders up to n=3348): `ipc.get_peaks` ~2 ms; FE wait/paint seconds; FPS 1–12 while busy, ~60 idle. Wave shimmer with empty bottom bar = FE peaks queue, not Rust analyze.
...
1. **FE peaks queue flood (primary scroll/waves issue).** `ROW_OVERSCAN=40` + warm prefetch + no cancel. Two IPC in flight → `q_wait` hundreds → shimmer + main-thread stalls. Rust is not the bottleneck.
2. **Huge list payloads.** `limit=5000`; `fe.list_apply` can hit hundreds of ms–seconds for n≈3k. Virtualizer limits DOM nodes, not React state / commit cost.
...
4. **RSS to ~12 GB** after mass decode; does not return to a lean baseline. Decode LRU is only 16; FE `peakCache` is unbounded (~1k entries seen). Suspect native allocator retention after ~14k full-file decodes + WebView heap. Hurts resize/scroll globally.
...
1. Pause peaks enqueue while scrolling; resume ~100–150 ms after scroll idle.
...
4. Cap list page size (~200–500), not 5000.
...
12. Periodic cloud→local recheck while open.
...
### Confidence: evidence-based, not hope

This is **not** a scattershot “tweak and pray” list. Each P0 maps to a measured cause:

| Symptom | Evidence | Fix that should work |
| --- | --- | --- |
| Slow scroll / shimmer waves | `ipc.get_peaks` ~2 ms vs FE wait seconds; `q_wait` 150–500+ | Cancel + pause peaks; cut overscan (industry default overscan is ~1–5, we use **40**) |
| Lag opening big folders | `fe.list_apply` hundreds of ms–seconds for n≈3k with `limit=5000` | Page/window from SQLite; virtualization alone does not shrink React state |
| Lag during analyze | 550× `folder_tree`; FPS 1–12 then ~60 when idle | Stop UI refresh storms; lower analyze concurrency while interacting |
...
Apps that feel fine at tens of thousands of samples (Wave Silo, Scanalyzer-style browsers, DAW browsers) do the same shape we already started: **index in SQLite, analyze in background, store peak sidecars, virtualize the list, only paint viewport waveforms**. They do **not** keep a 5k-row React array hot or prefetch hundreds of canvases ahead of scroll.
...
15. **Windowed SQL / infinite scroll**, not only a smaller `limit` once: TanStack docs stress virtualization ≠ holding the full dataset in the client.
16. **Batch `get_peaks` for the visible range** (one IPC, many ids) to cut bridge overhead vs 2-at-a-time singles.
17. **Analyze concurrency 1–2 while the window is focused** (4 full-file decodes × large WAVs = multi‑GB working set even with perfect frees).
...
| `ipc.list_samples` | 4 | 9–21 ms | Includes n=5000 |
